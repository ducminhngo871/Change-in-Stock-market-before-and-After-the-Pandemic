...
```
The scale of each sector are different by the power of 10; therefore, the graph display an enormous part as Finance while the rest of the sectors as tiny part and can be barely seen. 
```{r }
sp %>% 
  group_by(Sector) %>% 
  summarise(cap = sum(market_cap))
```
...
p <- sp %>% 
  ...
  ggplot( aes(area = market_cap ,
              fill = company, 
              subgroup=sector, 
              label = company)) +
  geom_treemap(aes(alpha = shares)) +
    ...
    geom_treemap_subgroup_text(place = "centre",
                               ...
                               reflow=F,
                                         min.size = 0) +
  labs(title = "Distribution of Market Cap")+
  theme(
...
anim_save("sp_market_cap_treemap_anim.gif")
```
```{r Treemap Graph 2}
p2 <- sp %>% 
  clean_names() %>% 
  ...
  transition_time(date) +
  ease_aes('linear')
animate(p2, duration= 20, fps= 10, renderer = gifski_renderer())

anim_save("sp_market_cap_treemap_anim_without_financials.gif")
```
